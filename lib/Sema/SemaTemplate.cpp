...
void Sema::FilterAcceptableTemplateNames(LookupResult &R,
  ...
  while (filter.hasNext()) {
    ...
    NamedDecl *Repl =
        isAcceptableTemplateName(Context, Orig, AllowFunctionTemplates);
    if (!Repl)
      filter.erase();
    else if (Repl != Orig) {
...
NamedDecl *Sema::ActOnTypeParameter(
    Scope *S, bool Typename, SourceLocation EllipsisLoc, SourceLocation KeyLoc,
    IdentifierInfo *ParamName, SourceLocation ParamNameLoc, unsigned Depth,
    unsigned Position, SourceLocation EqualLoc, ParsedType DefaultArg) {
  ...
  TemplateTypeParmDecl *Param = TemplateTypeParmDecl::Create(
      Context, Context.getTranslationUnitDecl(), KeyLoc, Loc, Depth, Position,
...
void Sema::DeclareImplicitDeductionGuides(TemplateDecl *Template,
  ...
  if (auto *Alias = dyn_cast<TypeAliasTemplateDecl>(Template)) {
    auto TAD = cast<TypeAliasDecl>(Alias->getTemplatedDecl());
...
TypeResult Sema::ActOnTemplateIdType(
    ...
    IdentifierInfo *TemplateII, SourceLocation TemplateIILoc,
...
ExprResult Sema::CheckTemplateArgument(NonTypeTemplateParmDecl *Param,
  ...
  if ( // -- For a non-type template-parameter of type pointer to
      ...
      //    the set (13.4).
      (ParamType->isMemberPointerType() && ParamType->getAs<MemberPointerType>()
                                               ->getPointeeType()
                                               ->isFunctionType())) {
  ...
  if (VD->getDeclContext()->isRecord() &&
    ...
    if (ParamType->isMemberPointerType()) {
      ...
      ExprResult RefExpr = BuildDeclRefExpr(
          VD, VD->getType().getNonReferenceType(), VK, Loc, &SS);
...
static bool MatchTemplateParameterKind(
  ...
  if (TemplateTemplateParmDecl *OldTTP =
    ...
    return S.TemplateParameterListsAreEqual(
        ...
        Complain,
        (Kind == Sema::TPL_TemplateMatch ? Sema::TPL_TemplateTemplateParmMatch
                                         : Kind),
        TemplateArgLoc);
  }
...
DeclResult Sema::ActOnExplicitInstantiation(Scope *S, SourceLocation ExternLoc,
  ...
  CheckExplicitInstantiationScope(
      *this,
      ...
      D.getIdentifierLoc(), D.getCXXScopeSpec().isSet());
...
QualType Sema::CheckTypenameType(ElaboratedTypeKeyword Keyword,
  ...
  if (Referenced)
    Diag(Referenced->getLocation(), diag::note_typename_refers_here) << Name;
  return QualType();
}
